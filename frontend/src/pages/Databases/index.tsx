...
const Databases: React.FC = () => {
  const { data, error, mutate } = useFetch<Database[]>("api/database");
  ...
  const onSubmit: SubmitHandler<Database> = (r) => {
    api.post(`api/database-test`, r).then((response) => {
      if (response.data.status == "Error") {
        setMessage(response.data.message);
      }
      if (response.data.status == "OK") {
        api.post(`api/database`, r).then((response) => {
          if (response.data.status == "Error") {
            setMessage(response.data.message);
          } else {
            mutate(data);
  ...
  const onEdit: SubmitHandler<Database> = (r) => {
    api.post(`api/database-test`, r).then((response) => {
      ...
      if (response.data.status == "OK") {
        api.put(`api/database/${r.id}`, r).then((response) => {
          mutate(data);
          toggleEdit();
        });
      }
  ...
  const onDelete: SubmitHandler<Edit> = (r) => {
    api.delete(`api/database/${r.id}`).then((response) => {
      mutate(data);
      toggleDelete();
    });
  };
  ...
  const inputs = (reg: UseFormRegister<Database>) => (
    <>
      ...
      <FormGroup width="300px">
        ...
        <Input {...reg("password", { required: true })} type="password"></Input>
      </FormGroup>
      <FormGroup width="300px">
